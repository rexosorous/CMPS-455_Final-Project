...
class Compiler:
        ...
        try:
            while True:
                ...
                if read_value not in self.TERMINALS:        # if the word is not a terminal, try it as a variable name
                    if not after_declarations: # don't add variable names if we're past the declaration section
    ...
    def run(self, filename):
        ...
        print('all text printed below this line is generated by the code segment being run!!')
        print('-------------------------------------------------------------------------------------')
